...
// ContentAnalysisEvent::Create() passing it the agent created above.  An event represents
...
//
// The agent is not required to serialize event handling.  That is, once
// a event is created, the agent can analyze it in the background while
// calling ContentAnalysisEvent::Create() to receive the next content analysis request.
//
...
class BrowserInfo {
  long pid;  // Process of Google Chrome browser process.
};
...
class ContentAnalysisEvent {
 public:
  ...
  // sent to Google Chrome as the verdict for the request of this event.
  ...
  virtual const ContentAnalysisAcknowledgement& GetAcknowledgement() const = 0;
...
// actions that required for these events.  The default action is to do nothing
...
// respond to before this callback returns.  The agent may pass the ContentAnalysisEvent
// to a background task and respond when ready.  This callback has no default
// action and agents must override it.
//
...
//
// Agent instances should outlive all ContentAnalysisEvent instances created with it.
// Agent instances are not thread safe except for Stop() which can be called
// from any thread to shutdown the agent.  Outstanding ContentAnalysisEvents
// created from this agent may or may not still complete.
//
// See the demo directory for an example of how to use this class.
class Agent {
 public:
...
// function that is equivalent to the following:
//
//   SetEventVerdictTo(ContentAnalysisResponse::Result::TriggeredRule::BLOCK);
